function [varargout] = gp2(hyp, inf, mean, cov, lik, x, y, xs, ys)
...
if nargin==7                                     % if no test cases are provided
  ...
  nperbatch = 50000;                       % number of data points per mini batch
  ...
  while nact<ns               % process minibatches of test cases to save memory
    ...
    Kss = feval(cov{:}, hyp.cov, xs(id,:)); % covariance matrix ns*ns
    ...
    if Lchol    % L contains chol decomp => use Cholesky parameters (alpha,sW,L)
      ...
      fs2(id,id) = Kss - V'*V;                        % predictive covariance matrix
    else                % L is not triangular => use alternative parametrisation
      if isnumeric(L)
           LKs = L*Ks; 
      else
           LKs = L(Ks); % matrix or callback
      end    
      %fs2(id) = kss + sum(Ks.*LKs,1)';                    % predictive variances
      fs2(id,id) = Kss + Ks'*LKs;                    % predictive variances
    end
  ...
  if nargin<9
    varargout = {fmu, fs2, [], post};        % assign output arguments
  else
    varargout = {fmu, fs2, lp, post};
  end
end
